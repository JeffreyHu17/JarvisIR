...
class CycleGAN_Turbo(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4, device='cuda'):
        super().__init__()
        self.path = '' # XXX：Set to local path
        self.tokenizer = AutoTokenizer.from_pretrained(os.path.join(self.path, "stabilityai/sd-turbo"), subfolder="tokenizer")
        self.text_encoder = CLIPTextModel.from_pretrained(os.path.join(self.path, "stabilityai/sd-turbo"), subfolder="text_encoder").to(device)
        self.sched = make_1step_sched(self.path, device)
        vae = AutoencoderKL.from_pretrained(os.path.join(self.path, "stabilityai/sd-turbo"), subfolder="vae")
        unet = UNet2DConditionModel.from_pretrained(os.path.join(self.path, "stabilityai/sd-turbo"), subfolder="unet")
        vae.encoder.forward = my_vae_encoder_fwd.__get__(vae.encoder, vae.encoder.__class__)
        vae.decoder.forward = my_vae_decoder_fwd.__get__(vae.decoder, vae.decoder.__class__)
        # add the skip connection convs
