...
if __name__ == "__main__":
    resolution = 256
    net = create_atgan_nets().cuda()
    print(params_count(net))
    # print(net)
    with torch.no_grad():
        
        # x = torch.ones(4 * 6 * 3 * resolution * resolution).reshape(4, 6, 3, resolution, resolution)
        x = torch.ones(4 * 3 * resolution * resolution).reshape(4, 3, resolution, resolution).cuda()
        ...
        mask_list, frame1, frame2, a = net(x)
