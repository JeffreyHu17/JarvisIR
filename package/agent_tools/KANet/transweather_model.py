...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride,
                              padding=(patch_size[0] // 2, patch_size[1] // 2)).cpu()
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        # c=x.cpu()
        # x = self.proj(x.detach().cpu().numpy()).to('cuda')
        _, _, H, W = x.shape
...
class convprojection(nn.Module):
    def __init__(self, path=None, **kwargs):
        ...
        self.convd32x = UpsampleConvLayer(512, 512, kernel_size=2, stride=2)
        self.convd16x = UpsampleConvLayer(512, 320, kernel_size=2, stride=2)
        self.dense_4 = nn.Sequential(ResidualBlock(320))
        self.convd8x = UpsampleConvLayer(320, 128, kernel_size=2, stride=2)
        self.dense_3 = nn.Sequential(ResidualBlock(128))
        self.convd4x = UpsampleConvLayer(128, 64, kernel_size=2, stride=2)
        self.dense_2 = nn.Sequential(ResidualBlock(64))
        self.convd2x = UpsampleConvLayer(64, 16, kernel_size=2, stride=2)
        self.dense_1 = nn.Sequential( ResidualBlock(16))
        self.convd1x = UpsampleConvLayer(16, 8, kernel_size=2, stride=2)
