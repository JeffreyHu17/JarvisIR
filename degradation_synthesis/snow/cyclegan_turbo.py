...
# p = "src/"
# sys.path.append(p)
from snow.model import make_1step_sched, my_vae_encoder_fwd, my_vae_decoder_fwd, download_url
...
class CycleGAN_Turbo(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4):
        ...
        if pretrained_name == "day_to_night":
            ...
            self.caption = "driving in the day"
            self.direction = "b2a"
        elif pretrained_name == "clear_to_snow":
            sd = torch.load(pretrained_path)
            self.load_ckpt_from_state_dict(sd)
            self.timesteps = torch.tensor([999], device="cuda").long()
            self.caption = "driving in heavy snow"
            self.direction = "a2b"
